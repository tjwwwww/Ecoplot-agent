...
def _normalize_agent_artifacts(artifacts: Any, request: Request) -> List[Dict[str, Any]]:
    """
    将 agent2.py 返回的图表/文件统一变成前端可直接展示的 URL。
    兼容字段：
    - artifact_url / relative_url / url
    - path / file_path / image_path / output_path / chart_path / png_path / svg_path
    """
    if not isinstance(artifacts, list):
        return []

    normalized: List[Dict[str, Any]] = []
    path_keys = ["path", "file_path", "image_path", "output_path", "chart_path", "png_path", "svg_path"]
    ...
    for item in artifacts:
        ...
        # 如果是本地文件路径，复制/确认到 /visualizations
        ...
        # 如果 URL 已经是 /visualizations/xxx，直接保留
        if isinstance(rel, str) and rel.startswith("/visualizations/"):
            data["artifact_url"] = rel
            data["artifact_absolute_url"] = str(request.base_url).rstrip("/") + rel
            ...
            # 相对路径尝试转成 /visualizations/<filename>
            name = Path(rel).name
            if name:
                candidate = VISUALIZATION_DIR / name
                if candidate.exists():
                    data["artifact_url"] = f"/visualizations/{name}"
                    data["artifact_absolute_url"] = str(request.base_url).rstrip("/") + f"/visualizations/{name}"
...
def _run_agent2(payload: AgentChatRequest, context: Dict[str, Any], mode: str, event_callback=None) -> Dict[str, Any]:
    ...
    """
    try:
        from agent2 import run_agent_chat, run_agent_report
    except Exception as exc:
        raise HTTPException(status_code=503, detail=f"Agent2 backend unavailable: {exc}") from exc
...
def agent_health() -> Dict[str, Any]:
    ...
    """
    modules = {}
    for name in ["agent2", "ontology_retriever", "semantic_planner", "domain_analysis_engine"]:
        try:
            __import__(name)
...
def agent_chat_stream(payload: AgentChatRequest, request: Request, authorized: bool = Depends(verify_api_key)) -> StreamingResponse:
    """
    轻量 NDJSON 流式接口。
    当前实现重点是前端可看到 start / tool events / final；真正逐 token 流式可后续再增强。
    """
    ...
    def _iter_stream():
        ...
        def _push(event_type: str, data: Dict[str, Any]) -> None:
            emitted.append(_json.dumps({"type": event_type, "data": data}, ensure_ascii=False) + "\n")

        yield _json.dumps({"type": "start", "data": {"question": payload.question, "scope": scope, "mode": mode}}, ensure_ascii=False) + "\n"

        details = _run_agent2(payload, context=context, mode=mode, event_callback=_push)
        ...
        for idx in range(0, len(answer), 64):
            yield _json.dumps({"type": "answer_chunk", "data": {"text": answer[idx: idx + 64]}}, ensure_ascii=False) + "\n"
            _time.sleep(0.005)
        ...
        yield _json.dumps({"type": "final", "data": final_payload}, ensure_ascii=False) + "\n"
...
def api_plot_climate(request: Request) -> Dict[str, Any]:
    _require_visualization_engine()
    return _normalize_plot_result(plot_climate_time_series(), request)
